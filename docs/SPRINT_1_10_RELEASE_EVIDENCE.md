...
| 7      | CONDITIONAL | Strict compiled identity/assets/store/link profiles, signed ETag-aware tenant-pinned cache, isolated App Ops build lifecycle and history, Ops screens, production placeholder block, and successful iOS/Android/web synthetic exports for two distinct hotel apps                                             | Hosted checks, prior sprint merges, protected-main approval, approved assets, signing, and native binary/device QA remain.                   |
