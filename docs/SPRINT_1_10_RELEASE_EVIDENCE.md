...
| 4      | CONDITIONAL | Ordered review complete; local `ci:verify`, 32 foundation controls, PostgreSQL migration concurrency, immutable deploy layouts, AWS Terraform, OIDC workflow, ADOT/X-Ray propagation, Sentry, health, alarms, WAF/TLS, and encrypted RDS/Redis/S3/secrets                                                | Protected PR approval plus authorized AWS dev configuration and a CI deployment with health/X-Ray evidence remain external gates.            |
