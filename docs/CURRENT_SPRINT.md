...
**Status:** CONDITIONAL — implementation and complete local exit gate pass; stacked protected-PR evidence and the Sprint 4 dependency gate remain
...
1. Push the stacked Sprint 5 branch and retain all four protected hosted CI results.
2. Obtain the independent approval required by protected `main`; do not weaken branch protection.
3. Complete Sprint 4 first: authorize/configure AWS dev and retain the CI deployment, PostgreSQL health, service stability, and matching X-Ray evidence.
4. Rebase/retarget Sprint 5 after Sprint 4 merges, rerun the protected checks, then merge in dependency order.

The repository-controlled Sprint 5 exit gate passes locally. Sprint 5 remains conditional because its protected stacked PR and prerequisite Sprint 4 external gates are not yet complete. Sprint 6 has not started.
