...
4. Create a customer-managed KMS key and one Secrets Manager JSON secret. Required keys are `BOOTSTRAP_PRIVATE_KEY_HEX`, `EMAIL_LOOKUP_HMAC_SECRET`, `PII_ENCRYPTION_KEY_BASE64`, `OTP_PEPPER`, `GUEST_JWT_SECRET`, `STAFF_JWT_SECRET`, `DATABASE_RUNTIME_PASSWORD`, and `SENTRY_DSN`. The workflow receives only the secret and key ARNs; values never enter GitHub or Terraform state. The password is at least 32 random bytes and is used only to configure/connect the non-owner `staybuddy_runtime` role; API and worker never receive the RDS master username or password.
